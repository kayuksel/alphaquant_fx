...
def technical_indicator(ohlcv: torch.Tensor) -> torch.Tensor:
    ...
    gain = dv.clamp_min(0)
    loss = (-dv).clamp_min(0)
    rsi_norm = torch.full((n,), 0.5, device=ohlcv.device, dtype=rts.dtype)
    if gain.size(1) >= 14:
        avg_gain = gain.unfold(1, 14, 1).mean(dim=2)[:, -1]
        avg_loss = loss.unfold(1, 14, 1).mean(dim=2)[:, -1].clamp_min(eps)
    ...
    med_last = rp2[:, -1].median()
    cross = (-(8 * (rp2[:, -1] - med_last) / (rts.std(dim=1) + eps))).sigmoid()
